...
class M4(DataPointSelector):
    """Aggregation method that selects the 4 M-s: y-argmin, y-argmax, x-argmin and x-argmax per bin
    
    !!! note

        When `n_out` is 4 * the canvas's pixel width, it should create a pixel-perfect visualization
        w.r.t. the raw data.

    !!! note
        
        When used for candlestick data, the 4 M-s represent the following values:
            - y-argmin = low
            - y-argmax = high
            - x-argmin = open
            - x-argmax = close
    ...
    def _arg_downsample(self, x: np.ndarray | None, y: np.ndarray, n_out: int) -> np.ndarray:
        if x is None:
        ...
        bins = M4._get_bin_idxs(x, n_out // 4)
